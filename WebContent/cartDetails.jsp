	...
	<div class="container">
		...
		<table class="table table-hover">
			...
			<tbody
				...
				double totAmount = 0;
				double totDiscount = 0;  // NEW: Track total discount
				for (CartBean item : cartItems) {
				    ...
				    // UPDATED: Calculate with discount
				    double originalPrice = product.getProdPrice() * prodQuantity;
				    double discountedPrice = product.getDiscountedPrice() * prodQuantity;
				    double itemDiscount = originalPrice - discountedPrice;
				    
				    double currAmount = discountedPrice;  // KEPT: currAmount now holds discounted price
				    
				    totAmount += currAmount;        // Use currAmount
				    totDiscount += itemDiscount;    // NEW: Add to total discount
				...
				<tr style="background-color: #f0f0f0;">
				    <td colspan="6" style="text-align: center; font-weight: bold;">Subtotal (Before Discount)</td>
				    <td style="text-decoration: line-through;">Rs <%=(totAmount + totDiscount)%></td>
				</tr>
				<tr style="background-color: #d4edda; color: #155724;">
				    <td colspan="6" style="text-align: center; font-weight: bold;">Thanksgiving Discount (20% off on select items)</td>
				    <td>- Rs <%=totDiscount%></td>
				</tr>
				<%
				}
				%>
				<tr style="background-color: grey; color: white;">
				    <td colspan="6" style="text-align: center; font-weight: bold; font-size: 16px;">Total Amount to Pay (in Rupees)</td>
				    <td style="font-weight: bold; font-size: 16px;">Rs <%=totAmount%></td>
				</tr>
				<%
				if (totAmount != 0) {
				%>
